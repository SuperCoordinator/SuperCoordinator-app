...
public class SFEE_transport_monitor_old {
    private SFEE sfee;
    ...
    public SFEE_transport_monitor_old() {
    }

    public SFEE_transport_monitor_old(SFEE sfee, SFEI previousSFEI, SFEI nextSFEI) {
        this.sfee = sfee;
...
//                boolean sfee_outSensor = (int) sensorsState.get(1).get(sfee.getOutSensor().getBit_offset()) == 1;
                if (utils.getInstance().getLogicalOperator().FE_detector(b_outSensor, SFEI_old_outSensors)) {
                    if (sfeiTransport.getPartsATM().size() > 0) {
                        ...
                        // Database space
                        // update part status
                        dbConnection.getInstance().getParts().update_status(p.getId(), serializer.getInstance().scene.toString(), part.status.IN_PRODUCTION.toString());
